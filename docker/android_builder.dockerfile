...
RUN DEBIAN_FRONTEND="noninteractive" apt-get install -y git-core gnupg flex bison build-essential python3 python3-pip python3-setuptools python3-wheel python3-mako \
	zip curl zlib1g-dev gcc-multilib g++-multilib libc6 libc6-dev-i386 lib32ncurses5-dev \
    x11proto-core-dev libx11-dev lib32z1-dev libgl1-mesa-dev libxml2-utils xsltproc unzip \
	fontconfig libncurses5 procps rsync gperf ccache squashfs-tools libssl-dev ninja-build lunzip \
...
RUN groupadd -g $groupid $username && useradd -m -s /bin/bash -u $userid -g $groupid $username &&  usermod -aG sudo $username
...
RUN pip3 install meson==1.6.1 pycparser ply PyYAML
